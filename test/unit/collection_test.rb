...
module AePageObjects
  class CollectionTest < Test::Unit::TestCase
    ...
    def test_empty
      ...
      magazine_node.stubs(:all).with(:xpath, magazine.send(:item_xpath)).returns([])
    ...
    def test_non_empty
      ...
      bullet1_stub = mock
      bullet2_stub = mock
      magazine_node.stubs(:all).with(:xpath, magazine.send(:item_xpath)).returns([bullet1_stub, bullet2_stub])

      assert_equal 2, magazine.size
      
      magazine_node.expects(:find).with(:xpath, "#{magazine.send(:item_xpath)}[1]").returns(bullet1_stub)
      magazine_node.expects(:find).with(:xpath, "#{magazine.send(:item_xpath)}[2]").returns(bullet2_stub)
      ...
      magazine_node.expects(:find).with(:xpath, "#{magazine.send(:item_xpath)}[1]").times(2).returns(bullet1_stub)
      assert_equal bullet1_stub, magazine.at(0).node
      assert_equal bullet1_stub, magazine.first.node
            
      magazine_node.expects(:find).with(:xpath, "#{magazine.send(:item_xpath)}[2]").times(2).returns(bullet2_stub)
